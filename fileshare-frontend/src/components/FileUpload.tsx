                ...
                <div className={"w-full h-full flex justify-center align-center mb-1"}>
                    <svg width="60" height="60" viewBox="0 0 50 50" className="loading-dots">
                        <circle cx="10" cy="25" r="2" className="dot">
                            <animate attributeName="cy" values="25;20;25;30;25" dur="1s" begin="0s"
                                     repeatCount="indefinite"/>
                            <animate attributeName="opacity" values="1;0.3;1" dur="1s" begin="0s"
                                     repeatCount="indefinite"/>
                        </circle>

                        <circle cx="18" cy="25" r="2" className="dot">
                            <animate attributeName="cy" values="25;20;25;30;25" dur="1s" begin="0.1s"
                                     repeatCount="indefinite"/>
                            <animate attributeName="opacity" values="1;0.3;1" dur="1s" begin="0.1s"
                                     repeatCount="indefinite"/>
                        </circle>

                        <circle cx="26" cy="25" r="2" className="dot">
                            <animate attributeName="cy" values="25;20;25;30;25" dur="1s" begin="0.2s"
                                     repeatCount="indefinite"/>
                            <animate attributeName="opacity" values="1;0.3;1" dur="1s" begin="0.2s"
                                     repeatCount="indefinite"/>
                        </circle>

                        <circle cx="34" cy="25" r="2" className="dot">
                            <animate attributeName="cy" values="25;20;25;30;25" dur="1s" begin="0.3s"
                                     repeatCount="indefinite"/>
                            <animate attributeName="opacity" values="1;0.3;1" dur="1s" begin="0.3s"
                                     repeatCount="indefinite"/>
                        </circle>

                        <circle cx="42" cy="25" r="2" className="dot">
                            <animate attributeName="cy" values="25;20;25;30;25" dur="1s" begin="0.4s"
                                     repeatCount="indefinite"/>
                            <animate attributeName="opacity" values="1;0.3;1" dur="1s" begin="0.4s"
                                     repeatCount="indefinite"/>
                        </circle>
                    </svg>
    ...
    if (file.isComplete) {
        return (
            <div className="text-center space-y-6 animate-in fade-in duration-300">
                <div
                    className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-surface-hover text-green-500 mb-2">
        ...
        return (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex justify-between items-center text-xs text-secondary px-1">
                    <span
                        className="hidden sm:block">{file.selectedFiles.length} {file.selectedFiles.length > 1 ? 'files' : 'file'} selected • Total size: {formatBytes(file.totalFilesSize)}</span>
                    <span
                        className="sm:hidden">{file.selectedFiles.length} {file.selectedFiles.length > 1 ? 'files' : 'file'} • {formatBytes(file.totalFilesSize)}</span>
                    <button onClick={handleReset}
                            className="hover:text-red-500 transition-colors cursor-pointer">Cancel
                    </button>
                </div>
          ...
          <span
              className="inline-flex items-center gap-2 px-3 py-1 bg-surface rounded-full text-xs text-secondary border border-border">
            <span
                className={`w-1.5 h-1.5 rounded-full ${p2p.isConnected ? 'bg-green-500' : 'bg-amber-500 animate-pulse'}`}/>
              {p2p.status}
          </span>
                </div>
            </div>
    ...
    return (
      ...
      `}
        >
            <input {...getInputProps()} />
            <div
                className="p-3 bg-surface rounded-full mb-3 group-hover:scale-110 transition-transform duration-300 border border-border group-hover:border-secondary">
